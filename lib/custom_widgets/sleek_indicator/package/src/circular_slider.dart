...
class _SleekCircularSliderState extends State<SleekCircularSlider> with SingleTickerProviderStateMixin {
  ...
  double? _sectionAngle; /// 터치 구간에 따른 Anlge 설정 (custom UI)
  ...
  void _setupPainter({bool counterClockwise = false}) {
    ...
    _painter = _CurvePainter(
        ...
        value : angleToValue(_currentAngle!, widget.min, widget.max, _angleRange)
    );
  ...
  Widget? _buildChildWidget() {
    ...
    final childWidget = widget.innerWidget != null
        ...
        ? widget.innerWidget!(_currentAngle!)
        : SliderLabel(
            value: value,
            appearance: widget.appearance,
          );
  ...
  void _handlePan(Offset details, bool isPanEnd) {
    ...
    if (isPointAlongCircle(
      ...
      // print("_selectedAngle touch 앵글 : "+coordinatesToRadians(_painter!.center!, position).toString());
      // setup painter with new angle values and update onChange
      /// 각도 변경에 따른 UI 변경
      // _setupPainter(counterClockwise: widget.appearance.counterClockwise); // UI 변경 함수 호출
      _customTicTocPainter(counterClockwise: widget.appearance.counterClockwise); // 커스텀 UI 변경 함수 호출
      _updateOnChange();  // onChange 함수 return 값 설정
      setState(() {});
    }
  }
  ...
  bool _onPanDown(Offset details) {
    ...
    // print("_onPanDown touch 앵글 : "+coordinatesToRadians(_painter!.center!, position).toString());
...
/// 커스텀 코드 ------------------------------------------------------------------------------------
  void _customTicTocPainter({bool counterClockwise = false}) {
    var defaultAngle = _currentAngle ?? widget.angle;
    if (_oldWidgetAngle != null) {
      if (_oldWidgetAngle != widget.angle) {
        _selectedAngle = null;
    ...
    /// 터치 구간별 Angle
    _sectionAngle = angleBySection(currentAngle: _currentAngle ?? 0.0);
    _painter = _CurvePainter(
        startAngle: _startAngle,
        angleRange: _angleRange,
        angle: _sectionAngle! < 0.5 ? 0.5 : _sectionAngle!,
        appearance: widget.appearance,
        value : angleToValue(_sectionAngle!, widget.min, widget.max, _angleRange)
    );
    _oldWidgetAngle = widget.angle;
    _oldWidgetValue = widget.initialValue;
  }
